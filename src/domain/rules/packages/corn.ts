import { classifyMoisture, hasRecentObservation, hasRecentOperation, IRRIGATION_LOOKBACK_MS, latestObservationOfType, latestPhenologyText, makeRecommendation, normalizeText, readStringField, SCOUT_LOOKBACK_MS, SOIL_TEST_LOOKBACK_MS, WATER_SIGNAL_LOOKBACK_MS } from "../helpers.js";
  ...
  evaluateNutrition(ctx) {
    ...
    if (soilInterpretation.includes("low") || soilInterpretation.includes("deficien")) {
      return [
        makeRecommendation(ctx, {
          ...
          confidence: 0.76,
          rationale:
            `Corn target yield is ${ctx.cropPlan.targetYield}, and the latest soil test suggests a low or deficient nutrient condition.`,
          proposedActions: [
    ...
    return [
      makeRecommendation(ctx, {
        ...
        confidence: 0.61,
        rationale:
          `Corn has both targetYield (${ctx.cropPlan.targetYield}) and a soil test on record, so nutrition decisions can be tied to structured evidence rather than ad hoc guesses.`,
        proposedActions: [
